...
def add_reed_solomon_error_correction(data):
    ...
    encoded_data = rs.encode(data)  # Directly encode the byte data
    return encoded_data

def remove_reed_solomon_error_correction(data):
    rs = reedsolo.RSCodec(10)
    decoded_data = rs.decode(data)  # Decode the data
    return decoded_data  # Return as bytes
...
# Calculate overhead
original_size = len(plaintext)  # Corrected line
...
def display_metrics():
    ...
    print(f"Encoded Size (bytes): {encoded_size}")
